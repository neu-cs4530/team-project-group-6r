...
export default function ReadPost({ post, toggleEdit, closeReadPost }: ReadPostProps): JSX.Element {
    ...
    useEffect(() => {
        socket?.on(`comments:${post.id}`, (newComments: ServerComment[]) => {
            if (setComments) setComments(newComments);
        })
        return () => {
            socket?.off(`comments:${post.id}`)
    ...
    const postBody = useMemo(() => (
        <Box width='100%'>
            ...
            <MultiMediaDisplay source={`${process.env.REACT_APP_TOWNS_SERVICE_URL}/streamfile/${post.file?.filename}`} mimetype={post.file?.contentType} />
            <Text fontSize='md'
                maxHeight='145px'
                overflow='auto'
                overflowX='hidden'
